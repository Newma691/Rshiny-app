...
stocks_w = read.csv('stocks_w.csv', stringsAsFactors = F) 
...
ui <- fluidPage(
  dashboardPage (
    skin = "green",
    dashboardHeader(title = 'Stocks in High Frequency Trading', titleWidth = 230),
    dashboardSidebar(
      sidebarUserPanel(h2("AAPL")),
      # sidebarUserPanel(h2("5min")),
      sidebarMenu(
        menuItem('Data', tabName = 'dt', icon = icon('database')),
        menuItem('TimeSeries', tabName = 'ts', icon = icon('line-chart'))
      ),
      
      selectizeInput('interval', h3('Interval'), 
                     choices = na.omit(unique(stocks_w$Interval)),
                     selected = '5min'),
      selectizeInput('symbols', h3('Select Stocks'), 
                     choices = unique(stocks_w$Name))
      ###########
    ),
    dashboardBody(
      tabItems(
        ...
        tabItem(tabName = 'dt',
                fluidRow(box(DT::dataTableOutput('table'),
                             h3('High Frequency Trading data of different companies'),
...
server <- function(input, output, session) {
    output$table = DT::renderDataTable({
      a = getAlphaVantageData(
        ...
        outputType = "xts",
        apiKey = "916a645a06msh72981c16abd0752p1d8d42jsn3c2ba912ca7e",
        doSleep = TRUE
      ) 
      datatable(a, rownames = F)
    ...
    #model fitting
    fit_model = reactive({
        Arima(fit_data()[1:200,], order = c(input$ar, input$diff, input$ma))
    })
    #### price of stocks 
    output$fit_stock = renderPlot({
      if (input$diff == 0) {
        ggplot(data = fit_data(), aes(x = 1:nrow(fit_data()), y = fit_data())) +
          geom_line() + xlab('Time Period') +
          ggtitle(fit_data()) + theme_bw()
      } else {
    ...
    output$pacf = renderPlot({
      if (input$diff == 0) {
        ...
        pacf(diff(ts(fit_data()[1:200,]), differences = input$diff), main = '')
      }
    })
    ...
    output$f_v_d = renderPlot({
      ggplot() +
        ...
        geom_line(aes(x = 1:length(fitted(fit_model())),
                  ...
                  inherit.aes = F) +
        ggtitle(fit_data()) + xlab('Time Period') + ylab('Stock Price') +
        theme_bw()
    })
}
